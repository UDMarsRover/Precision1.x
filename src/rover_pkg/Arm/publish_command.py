...
get_command_sub = rospy.Subscriber(
    "control_command", Float32MultiArray, callback_arm_command
)

while not rospy.is_shutdown():
    time.sleep(0.2)  # slow down rate of publisher
    command_pub.publish(
        command
    )  # continously publishes command data received from controller for
    # arm to receive
