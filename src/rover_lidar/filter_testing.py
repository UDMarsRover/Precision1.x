...
class lidar_data_processor():
            ...
            if len(last_points) > 0:
                ...
                # If not within the threshold but the current object has points
                    ...
                    if min(current_object).dist != float('inf'):
                        ...
                        current_obj_angle = (360 / len(self.data)) * (current_object[int(len(current_object)/2)].idx)
                        self.objects.append(current_object)
    ...
    def process_points_to_objects(self):
        ...
        if len(self.objects) > 1:
            ...
            if self.compare(self.objects[0][len(self.objects[0]) - 1].dist, self.objects[len(self.objects) - 1][0].dist, self.depth_threshold) :
                self.combine_abstract_objects(self.objects[0], self.objects[len(self.objects) - 1])
                del self.objects[0]; del self.objects[len(self.objects) - 1]
